...
The **JSON files** are arranged in the following way 
Each file logs results in an organized way—track timings, cache hits, failures, and exactly how each DNS query was performed.

***

| Part      | JSON File(s)                              | Generated By                      | Description                                                                 |
|-----------|-------------------------------------------|-----------------------------------|------------------------------------------------------------------------------|
| PART_B    | 1PCAP.json, 2PCAP.json, ...               | host.py (default host code)        | Results for each host using the system resolver; latency, status, throughput and resolved names|
| PART_D    | PCAPX.json (Resolver_no_cache_singleserver/)| customresolver.py + host.py       | Full DNS step-by-step for each domain using the custom DNS server (no cache) (All the required entities in part D) |
| PART_E    | PCAPX_cache_multiserver.json (Resolver_Multiserver/)| customDNS_cache.py        | Experiments with multi-server & cache; logs cache hits/misses and  (All the required entities in part D) |
...
This project explores DNS querying in a simulated virtual network using Mininet.

The focus is on capturing, resolving, and analyzing DNS queries both with the system's default resolver and a custom iterative DNS resolver/server.

The assignment includes in-depth measurements of latency, throughput, cache hits, and the full resolution path for each query.
...
- **DNS_custom.py**  
  ...
  Used to compare with the default resolver in PART_B. No results generated, as it live listens for any requrest of name resolution
...
- **customresolver.py**  
  ...
  Stores per-query information asked in Part D.
...
- **Resolver_no_cache_singleserver/**  
  ...
  Marks all the information required by Part D of the question in json format in a heirarchical format.
  Note: Some of the queries are repeated 3-4 times, because of the timeouts and recalling by the system.
