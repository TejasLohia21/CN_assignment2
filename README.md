...
The **JSON files** are the heart of every experiment in this assignment.  
...
|-----------|-------------------------------------------|-----------------------------------|------------------------------------------------------------------------------|
| PART_B    | 1PCAP.json, 2PCAP.json, ...               | host.py (default resolver)        | Results for each host using the system resolver; latency, status, throughput |
| PART_D    | PCAPX.json (Resolver_no_cache_singleserver/)| customresolver.py + host.py       | Full DNS step-by-step for each domain using the custom DNS server (no cache) |
| PART_E    | PCAPX_cache_multiserver.json (Resolver_Multiserver/)| customDNS_cache.py        | Experiments with multi-server & cache; logs cache hits/misses and response times |
